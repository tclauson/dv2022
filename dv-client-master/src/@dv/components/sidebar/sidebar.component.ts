...
import { DvSidebarService } from './sidebar.service';
import { DvMatchMediaService } from '@dv/services/match-media.service';
...
export class DvSidebarComponent implements OnInit, OnDestroy {
  // Name
  @Input()
  name: string;
   ...
   * @param {DvMatchMediaService} _dvMatchMediaService
  ...
  constructor(
    ...
    private _dvMatchMediaService: DvMatchMediaService,
  ...
  ngOnInit(): void {
    ...
    this._dvConfigService.config
      ...
      .subscribe((config) => {
        this._dvConfig = config;
      });

    // Register the sidebar
    this._dvSidebarService.register(this.name, this);
  ...
  ngOnDestroy(): void {
    ...
    this._dvSidebarService.unregister(this.name);
  ...
  private _setupLockedOpen(): void {
    ...
    this._dvMatchMediaService.onMediaChange
      .pipe(takeUntil(this._unsubscribeAll))
      .subscribe(() => {
  ...
  private _showBackdrop(): void {
    ...
    this._backdrop.classList.add('dv-sidebar-overlay');

    // Add a class depending on the invisibleOverlay option
    if (this.invisibleOverlay) {
      this._backdrop.classList.add('dv-sidebar-overlay-invisible');
    }
